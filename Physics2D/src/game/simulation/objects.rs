...
pub struct Rectangle {
    center_of_mass: Vec2,
    circle_hit_box: (Vec2, f64), // (centre, radius)
    vertices: Vec<[f64; 2]>,
    mass: f64,
    velocity: Vec2,
    potnrg: f64,
...
impl Rectangle {
    pub fn new(center: Vec2, vertices: Vec<[f64; 2]>, mass: f64) -> Rectangle {
        Rectangle {
            ...
            circle_hit_box: approx_circle_hitbox(&vertices),
            vertices,
            mass,
...
fn calc_mass_center(vert: &Vec<[f64; 2]>) -> Vec2 {
    ...
    while vertices.len() > 3 {
        for i in 0..vertices.len() {
            ...
            let temp = calc_triangle_mass_center_and_area([vertices[i], vertices[iplus1], vertices[iplus2]]);
...
fn calc_triangle_mass_center_and_area(vertices: [[f64; 2]; 3]) -> (Vec2, f64) {
    let length1 = Vec2::new(vertices[1][0] - vertices[0][0], vertices[1][1] - vertices[0][1]).length();
    let length2 = Vec2::new(vertices[2][0] - vertices[0][0], vertices[2][1] - vertices[0][1]).length();
    let length3 = Vec2::new(vertices[1][0] - vertices[2][0], vertices[1][1] - vertices[2][1]).length();
    let semi_perimiter = (length1 + length2 + length3)/ 2.0;
    let area = (semi_perimiter*(semi_perimiter - length1)*(semi_perimiter - length2)*(semi_perimiter - length3)).sqrt();
    let centroid = Vec2::new((vertices[0][0] + vertices[1][0] + vertices[2][0]) / 3.0, (vertices[0][1] + vertices[1][1] + vertices[2][1]) / 3.0);
    return (centroid * area, area);
}

fn approx_circle_hitbox(vertices: &Vec<[f64; 2]>) -> (Vec2, f64) {
    return (Vec2::new(0.0, 0.0), 0.0);
}
